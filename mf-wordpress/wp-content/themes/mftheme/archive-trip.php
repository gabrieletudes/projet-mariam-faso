...
<div class="section-wrapper section-wrapper--almost-white">
    ...
    <?php while( $the_query->have_posts() ) : $the_query->the_post();?>
        <article class="events-section__article ">
            ...
            <p class="events-section__article-content"><?php mf_the_excerpt(100,'trip_description'); ?></p>
            <a href="<?php the_permalink() ?>" class="view-more-link"><?=__('Lire plus sur le voyage ','mf');?><span class="hidden"><?=the_field('page_title');?></span></a>
        </article>
        <?php endwhile; ?>
    </section>
    <div class="button-wrapper button-wrapper--top button-wrapper--on-inversed">
        ...
        <?php if ($total_pages != 1 ):?>
            <div class="view-link--to-right">
                <?php previous_posts_link('Voyages précédents');?>
                <?php next_posts_link('Voyages suivants', $the_query->max_num_pages); ?>
            </div>
        <?php endif;?>
    </div>
<?php else:?>
    <p><?= __('vous essayez d’acceder a une page inexistante','mf');?></p>
    <a href="<?php the_permalink();?>" class="cta-button cta-button--on-inversed"><?= __('Revenir au début de la page des voyages.','');?></a>
<?php endif; ?>
</div>
